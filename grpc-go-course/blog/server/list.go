...
func (s *Server) ListBlogs(in *emptypb.Empty, stream pb.BlogService_ListBlogsServer) error {
	...
	// Find all documents in the collection with empty filter
	...
	// Close the cursor at the end of the function
	defer cur.Close(context.Background())

	// Iterate through the cursor and send each document to the stream
	for cur.Next(context.Background()) {
		...
		// If cursor has error, return error
